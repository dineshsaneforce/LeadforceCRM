<style>
    ...
    <form id="ProjectProdcutForm" method='post' name="LeadProdcutForm" action="<?php echo admin_url('projects/savedealproducts/'.$project->id); ?>">
        ...
        <div class="css-table">
            ...
            <div class="css-table-row" id="particularsrowfooter">
                ...
                <div class="text-right" style="padding-top:30px"><span id="stotal">
                    ...
                    </span><span id="suptotal"><?php echo $tax_val; ?></span><b><span id="grandtotal"><?php echo number_format($project_cost, 2); ?></span></b>
                </div>
            </div>
            <input type="hidden" name="grandtotal" id="gtot" value="<?php echo $project_cost; ?>">
        </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        $('#ProjectProdcutForm').submit(function(e){
            e.preventDefault();
            $.ajax({
                type:'POST',
                ...
                success: function(response){
                    if(response.success ==true){
                        $('#project_items_count').html(response.itemscount);
                        alert_float('success', response.msg);
                        setTimeout(function(){
                            document.getElementById('overlay_12').style.display = 'none'; 
                            window.location.href = admin_url+'projects/view/'+<?php echo $project->id ?>+'/?group=project_items'
                        },1000);
                    }
                }
            })
