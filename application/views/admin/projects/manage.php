...
<style>
	...
	.ui-sortable .form-check-input:checked {
					...
					<div class="panel-body">
						<div class="_buttons">
							...
							if (!is_admin(get_staff_user_id())) {
								...
								$forecast_url = admin_url('projects/kanbans_forecast?pipelines=' . $pipelines[0]['id'] . '&member=' . $mem . '&gsearch=' . $gsearch);
								$approval_url = admin_url('projects/index_list?approvalList=1&pipelines=' . $pipelines[0]['id'] . '&member=&gsearch=');
							}
							?>
									...
									<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
										<div class="modal-body">
											...
											<div class="form-group">
												<p class="text-muted">Reorder Columns</p>
												...
												<ul id="sortable">
													...
													?>
															<li class="ui-state-default text-muted">
																<input type="checkbox" class="form-check-input" name="settings[projects_list_column][<?php echo $ckey; ?>]" value="1" checked="checked" /> <span class="mleft5"><?php echo _l($colarr[$ckey]['ll']); ?></span>
																<span style="float:right"><i class="fa fa-th" aria-hidden="true"></i></span>
															</li>
													<?php }
													} ?>
							...
							<?php if (!empty($need_fields) && (in_array("status", $need_fields))) { ?>
								<div class="btn-group pull-right mleft4 btn-with-tooltip-group _filter_data" data-toggle="tooltip" data-title="<?php echo _l('filter_by'); ?>">
									...
									<?php foreach ($statuses as $status) { ?>
										<li class="<?php if ($status['filter_default'] == 1 && !$this->input->get('status') || $this->input->get('status') == $status['id']) {
														echo 'active';
													} ?>">
			...
			<div class="modal-body">
				...
				?>
						<div class="form-group select-placeholder" style="margin-top:10px;">
							<label for="status" class="control-label required"><?php if (in_array($need_field12, $mandatory_fields1) || $req_field == 'name') { ?><small class="req text-danger">* </small><?php  }
																																																	echo _l($need_fields_label[$i]); ?></label>
							...
							<?php } else if ($need_field12 == 'project_contacts[]') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select id="<?php echo $need_fields_label[$i]; ?>" name="sel_<?php echo $need_field12; ?>" data-live-search="true" data-width="100%" class="selectpicker " data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" multiple="true" onchange="get_primary_person()">
										<?php
							...
							<?php } else if ($need_field12 == 'primary_contact') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select id="<?php echo $need_fields_label[$i]; ?>" name="sel_<?php echo $need_field12; ?>" data-live-search="true" data-width="100%" class="selectpicker " data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
										<?php
							...
							<?php } else if ($need_field12 == 'pipeline_id') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select id="<?php echo $need_fields_label[$i]; ?>" name="sel_<?php echo $need_field12; ?>" data-live-search="true" data-width="100%" class="selectpicker " data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" onchange="get_stages1(this.value)">
										<option></option>
										<?php
										if (!empty($pipelines)) {
							...
							<?php } else if ($need_field12 == 'teamleader' || $need_field12 == 'teamleader_name') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select name="sel_<?php echo $need_field12; ?>" id="<?php echo $need_fields_label[$i]; ?>" class="selectpicker " data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
										<option></option>
							...
							<?php } else if ($need_field12 == 'project_members[]') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select name="sel_<?php echo $need_field12; ?>" id="<?php echo $need_fields_label[$i]; ?>" class="selectpicker " data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" multiple="true">
										<option></option>
										<?php foreach ($teamleaders as $teamleader1) { ?>
											<option value="<?php echo $teamleader1['id']; ?>"><?php echo $teamleader1['name']; ?></option>
										<?php } ?>
							...
							<?php } else if ($need_field12 == 'project_cost') { ?>
								<div id="div_<?php echo $need_fields_label[$i]; ?>" style="display:none;margin-top:15px;" class="ch_field_div">
									<select class="selectpicker ch_field" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" name="project_currency">
										<option></option>
										<?php foreach ($allcurrency as $currency) { ?>
											<option value="<?php echo $currency['name']; ?>" <?php if ($basecurrency->id == $currency['id']) {
																									echo 'selected';
																								} ?>><?php echo $currency['name'] . '(' . $currency['symbol'] . ')'; ?></option>
										<?php } ?>
									</select>
...
<script>
	...
	function edit_multiple() {
		...
		$.ajax({
			url: "<?php echo admin_url('projects/edit_multiple'); ?>?ids=" + all_val,
	...
	function get_stages(pipeline) {
		$.ajax({
			url: "<?php echo admin_url('projects/stages'); ?>?pipeline=" + pipeline,
			success: function(result) {
				var myArr = JSON.parse(result);
				$('#status').empty();
				$('#status').append(myArr.stages);
	...
	function get_person(org) {
		$.ajax({
			url: "<?php echo admin_url('projects/get_org_person'); ?>?org=" + org,
			success: function(result) {
				var myArr = JSON.parse(result);
	...
	function get_primary_person() {
		...
		$.ajax({
			url: "<?php echo admin_url('projects/get_cont_person'); ?>?person=" + person,
			success: function(result) {
				var myArr = JSON.parse(result);
				$('#project_primary_contacts').empty();
				$('#project_primary_contacts').append(myArr.persons);
